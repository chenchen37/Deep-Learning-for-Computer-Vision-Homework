...
train_dataset = DLCV_HW1_Q2_Dataset('../input/dlcv-hw1-q1-q2/hw1_data/p2_data/train/', train_tfm)
valid_dataset = DLCV_HW1_Q2_Dataset('../input/dlcv-hw1-q1-q2/hw1_data/p2_data/validation/', test_tfm)
...
for epoch in range(n_epochs):
    model.train()
    train_running_loss = []
    for i, (sat, mask, id) in enumerate(train_dataloader):
        sat = sat.to('cuda')
        mask = mask.to('cuda')
        pred = model(sat)['out']
    ...
    model.eval()
    valid_running_loss = []
    for i, (sat, mask, id) in enumerate(valid_dataloader):
        sat = sat.to('cuda')
        mask = mask.to('cuda')
        with torch.no_grad():
            pred = model(sat)['out']
    ...
    if valid_loss <= best_loss:
        ...
        best_epoch = epoch + 1
        na = str(epoch+1)
        torch.save(model.state_dict(), 'deeplabv3_resnet101_focalLoss_lr1e5_epoch'+na+'.ckpt')
    else:
